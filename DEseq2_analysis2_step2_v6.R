...
#subset ASP_DZP by those with different ancestral expression in salt and zinc:
ASP_EDZP_SZcst <- ASP_EDZP_same[which(ASP_EDZP_same$Names %in% SZcst_same$Names),] #28
...
## 8.2 sub pattern of plasticity switching ----
...
################################################################################################-
#9.0 Co-opted genes (parallel salt plasticity assimilated into mine adaptive expression) ----
################################################################################################-
...
#view(cooption_table1)

cooption_mean_simple_lineboxplot <- ggplot(cooption_table1, aes(x = eco_treat, y = log(mean_norm_count))) +
...
#copy of the above graph but edited to highlight the subsets 
# cooption_mean_lineboxplot1 <- ggplot(cooption_table1, aes(x = eco_treat, y = log(mean_norm_count), colour = pattern, fill = pattern)) +
#   #geom_errorbar(aes(ymin=log(max_norm_count), ymax=log(min_norm_count)), width=0.2, position = position_dodge(0.2)) +
#   geom_boxplot(outlier.shape = NA, fill = "gray80", colour = "grey40") +
#   #geom_boxplot(outlier.shape = NA, fill = "gray80") +
#   geom_line(aes(group = gene_name)) +
...
#both cooption and cue transfer in one graph. tbh prefer both separated.

# both_simple_lineboxplot1 <- ggplot(plast_facil_table, aes(x = eco_treat, y = log(mean_norm_count), colour = setname, fill = setname)) +
#   #geom_errorbar(aes(ymin=log(max_norm_count), ymax=log(min_norm_count)), width=0.2, position = position_dodge(0.2)) +
#   geom_boxplot(outlier.shape = NA, fill = "gray85", colour = "gray10") +
#   geom_line(aes(group = gene_name), colour = "gray40") +
#   #geom_point(colour = "blue4", fill = "blue", alpha = 0.6, cex = 4, shape = 21)+
#   geom_point(alpha = 0.6, cex = 4, shape = 21)+
#   theme_classic()+
#   theme(panel.background = element_blank(), 
#         strip.background = element_blank(), strip.placement = "outside", strip.text = element_text(size = 18),
#         aspect.ratio = 1.6, axis.title = element_text(size = 22), axis.text = element_text(size = 18))+
#   xlab("Ecotype and treatment")+
#   ylab("log of normalised gene counts")+
#   scale_colour_manual(values = c("blue4", "darkred"))+
#   scale_fill_manual(values = c("blue", "red"))+
#   scale_x_discrete(labels=c("cstS" = "Salt", "cstC" = "Control", "cstZ" = "Zinc",
#                             "minS" = "Salt", "minC" = "Control", "minZ" = "Zinc"))+
#   #facet_wrap(~ecotype, ncol = 2, scales="free_x", strip.position = "top")
#   #facet_grid(setname~ecotype, scales="free_x")
#   # facet_wrap2(~ecotype, ncol = 2, scales = "free_x", strip.position = "top",
#   #             labeller = labeller(ecotype = ecotype.labs), strip = strip)
#   facet_grid2(setname~ecotype, scales="free_x",
#              labeller = labeller(ecotype = ecotype.labs), strip = strip)
#both_simple_lineboxplot1


####
#graph for this ASP changes thing: 
# 
